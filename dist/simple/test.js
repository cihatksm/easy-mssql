...
database_1.default.Connect(config_1.sqlConfig, async (config, err) => {
    ...
    try {
        ...
        const dropProcedure = await database_1.default.Query('DROP PROCEDURE TestProcedure');
        if (dropProcedure) {
            console.log('✅ Stored procedure cleanup successful\n');
        }
        else {
            console.log('❌ Stored procedure cleanup failed\n');
            throw new Error('Stored procedure cleanup failed');
        }
    }
    catch (error) {
